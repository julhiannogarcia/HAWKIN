...
import { Volume2, VolumeX, Heart, Brain, Sparkles, ChevronRight, LayoutGrid, Palette, ArrowLeft, CheckCircle2, Trophy, Lightbulb, Rocket, Star, User } from 'lucide-react';
  ...
  return (
    <div className="flex-1 flex flex-col items-center">
      <AnimatePresence mode="wait">
        ...
        {/* VISTA 1: ONBOARDING (EDAD Y FASE) */}
        {view === 'onboarding' && (
          <motion.div 
            key="onboarding"
        ...
        {view === 'dashboard' && (
          ...
          >
             ...
             {filteredModules.length > 0 ? (
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {filteredModules.map((mod) => (
                    <button 
                      ...
                      className={`group p-10 rounded-[50px] ${mod.color} border-2 border-white hover:border-[#D5DBDB] transition-all text-left flex flex-col justify-between min-h-[420px] shadow-sm hover:shadow-2xl`}
                    >
                        ...
                        <div className="pt-8 border-t border-black/5 flex items-center justify-between">
                           <span className="text-[10px] font-black uppercase text-[#AAB7B8] tracking-widest">{mod.steps.length} Unidades</span>
                           <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center text-[#2E86C1] shadow-sm group-hover:bg-[#5DADE2] group-hover:text-white transition-all">
               ...
               <div className="p-20 text-center bg-white rounded-[60px] border-2 border-dashed border-[#D5DBDB] space-y-6">
                  <div className="text-6xl">🔭</div>
                  <h3 className="text-2xl font-bold text-[#7F8C8D]">Buscando más contenido...</h3>
                  <p className="text-[#BDC3C7] max-w-sm mx-auto">Nuestros ingenieros están cargando nuevos retos para esta categoría. Prueba otra fase mientras tanto.</p>
                  <button onClick={() => setPhase('Descubrimiento')} className="text-[#2E86C1] font-bold uppercase text-xs hover:underline">Volver a Descubrimiento</button>
               </div>
             )}
          </motion.div>
        )}
             ...
             {!isSuccess ? (
               <>
                 <header className="flex items-center justify-between">
                    <button 
                      onClick={() => setView('dashboard')}
                      className="flex items-center gap-3 text-[#7F8C8D] font-bold uppercase text-xs hover:text-[#2E86C1] transition-colors bg-white px-6 py-3 rounded-2xl shadow-sm border border-[#F2F4F4]"
                    >
               ...
               <motion.div 
                ...
                className="max-w-2xl w-full mx-auto bg-white rounded-[80px] p-20 text-center shadow-2xl border border-[#F2F4F4] space-y-12"
               >
                  <div className="relative inline-block">
                     <div className="absolute -inset-10 bg-[#27AE60] opacity-10 rounded-full animate-ping" />
                     <div className="w-48 h-48 bg-[#D5F5E3] rounded-full flex items-center justify-center relative z-10 shadow-inner">
                        <Trophy className="text-[#27AE60]" size={100} />
                     </div>
                  </div>
                  <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-6">
                     <div className="p-8 bg-[#F4F6F7] rounded-[40px] border border-[#D5DBDB]">
                        <p className="text-[10px] font-black text-[#BDC3C7] uppercase mb-2">XP Ganada</p>
                        <p className="text-4xl font-black text-[#2E86C1]">+500</p>
                     </div>
                     <div className="p-8 bg-[#F4F6F7] rounded-[40px] border border-[#D5DBDB]">
                        <p className="text-[10px] font-black text-[#BDC3C7] uppercase mb-2">Medalla</p>
                        <div className="flex justify-center text-[#F1C40F]"><Star size={40} fill="currentColor" /></div>
                     </div>
                  </div>
