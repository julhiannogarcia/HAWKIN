...
import {
  ...
  TrendingUp, FileText, UserCircle, Trophy, LogOut,
} from 'lucide-react';
      ...
      <aside className="w-72 border-r border-white/5 flex flex-col bg-[#020202] shadow-2xl relative z-20">
        ...
        <nav className="flex-1 overflow-y-auto p-6 flex flex-col gap-2 scrollbar-hide">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`flex items-center gap-4 p-4 rounded-2xl transition-colors text-[10px] font-black uppercase tracking-widest border ${
                pathname === link.href
                  ? 'bg-white/[0.04] text-white border-white/10'
                  : 'text-gray-400 border-transparent hover:bg-white/[0.02] hover:text-white hover:border-white/5'
              }`}
            >
              <div className="text-gray-600 group-hover:text-cyan-400">{link.icon}</div>
              {link.name}
            </Link>
          ))}
        </nav>
        ...
        <div className="p-6 border-t border-white/5 bg-white/[0.01] space-y-3">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[10px] font-black uppercase tracking-widest text-white">Sesión admin</p>
              <p className="text-[8px] font-bold text-gray-500 uppercase tracking-widest mt-1">Protegida por contraseña</p>
            </div>
          ...
          <button
            type="button"
            onClick={logout}
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg border border-white/10 text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-white hover:border-red-500/40"
          >
