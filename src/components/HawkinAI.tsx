...
export default function HawkinAI() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'ai', text: 'Socio, he tenido un pequeño desfase. Por favor, intenta de nuevo.' }]);
    } finally {
      setIsTyping(false);
    }
  };
        ...
        return (
          <div key={index} className="flex flex-wrap gap-2 my-3">
            {line.split('+').map((part, i) => (
              <span key={i} className="px-3 py-1.5 bg-white/10 border-b-2 border-white/20 rounded-lg font-black text-white text-[10px] shadow-md flex items-center gap-2">
              ...
              {messages.map((msg, i) => (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} key={i} className={`flex ${msg.role === 'ai' ? 'justify-start' : 'justify-end'}`}>
                  <div className={`max-w-[90%] p-4 rounded-3xl text-sm ${
                    msg.role === 'ai' 
                      ? 'bg-white/5 border border-white/10 text-gray-200 rounded-bl-none shadow-xl' 
