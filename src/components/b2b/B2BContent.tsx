...
export default function B2BContent() {
  ...
  useEffect(() => {
    ...
    const fetchGeo = async () => {
      try {
        ...
        setGeoData({ countryCode: 'PE', currencySymbol: 'S/', currencyCode: 'PEN', rate: 3.82, locale: 'es_PE' });
      }
    };
    fetchGeo();
  }, []);

  // 2. Carga Directa del SDK de PayPal
  useEffect(() => {
  ...
  // 3. Renderizado Forzado de Botones
  useEffect(() => {
    ...
    <div className="flex-1 flex flex-col items-center w-full min-h-screen bg-black overflow-x-hidden">
      ...
      <div className="max-w-6xl mx-auto px-6 pt-40 pb-32 w-full">
        ...
        <section className="text-center space-y-8 mb-32">
          ...
          <h1 className="text-6xl md:text-8xl font-black tracking-tighter leading-none italic text-white text-center uppercase">
          ...
          <p className="text-gray-400 text-xl max-w-2xl mx-auto font-light leading-relaxed text-center">
            Escala tu negocio con publicidad inteligente localizada por IP. 
          </p>
        </section>
              ...
              <div className="space-y-8 z-10 w-full">
                 ...
                 <div className="mt-8 bg-white/[0.02] border border-white/5 p-10 rounded-[45px] space-y-8">
                    <div className="flex justify-between items-end border-b border-white/5 pb-6">
                       <div>
                          <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1">Reserva de Espacio:</p>
                          <h5 className="text-lg font-bold text-cyan-400 uppercase italic underline leading-none">{selectedPlacement?.placement}</h5>
                       </div>
                       <div className="text-right">
                          <p className="text-2xl font-black text-white">{geoData?.currencySymbol || 'S/'}{((selectedPlacement?.price || 0) * (geoData?.rate || 3.82)).toFixed(0)}</p>
                       </div>
                    </div>
                    ...
                    <div className="min-h-[100px] flex items-center justify-center w-full">
                       {!isPaypalLoaded ? (
                         <div className="flex flex-col items-center gap-4 text-center">
                            ...
                            <p className="text-[10px] font-black text-gray-600 uppercase tracking-[0.3em] animate-pulse">Iniciando Pasarela Global...</p>
                         </div>
                       ) : (
